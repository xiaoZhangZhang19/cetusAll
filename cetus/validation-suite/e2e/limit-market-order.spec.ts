...
test.describe('Cetus Mainnet Limit Order (Market Price)', () => {
  test('places a SUI limit order at market price (~$5 worth)', async ({ page, walletController }) => {
    ...
    console.log(`[limit-market:e2e] SUI price (Pyth): $${suiPrice.toFixed(4)}`);
    console.log(`[limit-market:e2e] input amount     : ${inputAmount} SUI`);

    // ── Balance snapshot (before) ─────────────────────────────────────────────
    const balanceBefore = await getBalanceSnapshot(env.testWalletAddress, limitScenario.inputCoinType);
    console.log(`[limit-market:e2e] balance before   : ${fmtBalance(balanceBefore.totalBalance)}`);

    // ── Token selection ───────────────────────────────────────────────────────
    await limitPage.selectFromToken(limitScenario.inputCoinType);
    await limitPage.selectToToken(limitScenario.outputCoinType);
    // No price override — use the market price auto-filled by the UI
    ...
    if (digest) {
      console.log(`[limit-market:e2e] tx digest      : ${digest}`);
      expect(digest.length).toBeGreaterThan(10);
    } else {
      console.log('[limit-market:e2e] tx digest      : <not found in UI>');
    }
    ...
    if (!isNaN(expiryMs)) {
      ...
      console.log(`[limit-market:e2e] expiry offset  : ${diffHours.toFixed(2)} h from 7-day mark`);
    ...
    console.log(`[limit-market:e2e] balance after    : ${fmtBalance(balanceAfter.totalBalance)}`);
    console.log(`[limit-market:e2e] balance drop     : ${fmtBalance(decrease)}`);
