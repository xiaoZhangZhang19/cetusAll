...
function LogLine({ line, idx }: { line: string; idx: number }) {
  ...
  else if (/^\s*\[|^\s*>|^\s*#/.test(line))              cls = 'text-sky-500';
...
export function CetusLogModal({ title, subtitle, output, filename, onClose }: CetusLogModalProps) {
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = filename ?? `cetus-log-${Date.now()}.txt`;
    a.click();
  ...
  return (
    ...
    >
      <div
        className="flex max-h-[88vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl border border-sky-900/60 bg-slate-950 shadow-2xl shadow-sky-950/50"
        onClick={(e) => e.stopPropagation()}
      >
        {/* ── Header ── */}
        <div className="flex shrink-0 items-center justify-between border-b border-sky-900/40 bg-slate-900/80 px-5 py-3.5">
          <div className="flex items-center gap-3">
            {/* Cetus accent bar */}
            <div className="h-8 w-1 rounded-full bg-gradient-to-b from-sky-400 to-emerald-500" />
            <img
              src="https://app.cetus.zone/favicon.ico"
              alt="Cetus"
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg border border-sky-800/60 bg-sky-950/40 px-3 py-1.5 text-xs text-sky-400 transition hover:bg-sky-900/40 hover:text-sky-300"
            >
              ⬇ 下载日志
            </button>
            <button
